  ...
  <el-dialog
    ...
    <el-form :model="dataForm" :rules="dataRule" ref="dataForm" @keyup.enter.native="dataFormSubmit()" label-width="80px">
      ...
      <el-form-item label="工号" prop="workNo">
        <el-input v-model="dataForm.workNo" placeholder="工号"></el-input>
      </el-form-item>
    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/sys/teacher/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.userName = data.teacher.username
                this.dataForm.salt = data.teacher.salt
                this.dataForm.email = data.teacher.email
                this.dataForm.mobile = data.teacher.mobile
                this.dataForm.roleIdList = data.teacher.roleIdList
                this.dataForm.status = data.teacher.status
                this.dataForm.workNo = data.teacher.workNo
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/sys/teacher/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
